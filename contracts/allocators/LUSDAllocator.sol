...
contract LUSDAllocator is GretaAccessControlled {
    /* ======== DEPENDENCIES ======== */
    ...
    ITreasury public treasury; // Greta Treasury
    ...
    function withdraw(address token, uint256 amount) external onlyGuardian {
        ...
        if (token == lusdTokenAddress) {
            ...
            treasury.deposit(balance, token, value); // deposit using value as profit so no GRT is minted
        } else {
    ...
    /**
    Helper method copying GretaTreasury::_tokenValue(), whose name was 'valueOf()' in v1 
