...
import {IsGRT} from "../interfaces/IsGRT.sol";
import {SafeERC20} from "../libraries/SafeERC20.sol";
import {IYieldDirector} from "../interfaces/IYieldDirector.sol";
import {GretaAccessControlled, IGretaAuthority} from "../types/GretaAccessControlled.sol";

/**
    @title YieldDirector (codename Tyche) 
    @notice This contract allows donors to deposit their sGRT and donate their rebases
            to any address. Donors will be able to withdraw their principal
            sGRT at any time. Donation recipients can also redeem accrued rebases at any time.
 */
contract YieldDirector is IYieldDirector, GretaAccessControlled {
    ...
    struct DonationInfo {
        ...
        uint256 carry; // Amount of sGRT accumulated over on deposit/withdraw
    ...
    constructor (address sGrt_, address authority_)
        GretaAccessControlled(IGretaAuthority(authority_))
    {
        require(sGrt_ != address(0), "Invalid address for sGRT");
    ...
    ************************/
    ...
    /**
        @notice Deposit sGRT, records sender address and assign rebases to recipient
        @param amount_ Amount of sGRT debt issued from donor to recipient
    ...
    */
    function deposit(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sGRT).safeTransferFrom(msg.sender, address(this), amount_);

        uint256 index = IsGRT(sGRT).index();
    ...
    /**
        @notice Withdraw donor's sGRT from vault and subtracts debt from recipient
     */
    function withdraw(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sGRT).safeTransfer(msg.sender, amount_);
    ...
    function withdrawAll() external override {
        ...
        uint256 sGrtIndex = IsGRT(sGRT).index();
        uint256 total = 0;
        ...
        for (uint256 index = 0; index < donationsLength; index++) {
            ...
            recipient.agnosticDebt = _toAgnostic(recipient.totalDebt + recipient.carry);
            recipient.indexAtLastChange = sGrtIndex;
        ...
        IERC20(sGRT).safeTransfer(msg.sender, total);

        emit AllWithdrawn(msg.sender, total);
    }

    /**
        @notice Get deposited sGRT amount for specific recipient
     */
    ...
    /**
        @notice Return total amount of donor's sGRT deposited
     */
    ...
    /**
        @notice Return total amount of sGRT donated to recipient since last full withdrawal
     */
    ...
    /**
        @notice Return total amount of sGRT donated from donor since last full withdrawal
     */
    ...
    ************************/
    ...
    /**
        @notice Get redeemable sGRT balance of a recipient address
     */
    ...
    /**
        @notice Redeem recipient's full donated amount of sGRT at current index
        @dev Note that a recipient redeeming their vault shares effectively pays back all
             sGRT debt to donors at the time of redeem. Any future incurred debt will
    ...
    function redeem() external override {
        ...
        recipient.carry = 0;
        recipient.indexAtLastChange = IsGRT(sGRT).index();

        IERC20(sGRT).safeTransfer(msg.sender, redeemable);
    ...
    ************************/
    ...
    /**
        @notice Get accumulated sGRT since last time agnostic value changed.
     */
    ...
    /**
        @notice Convert flat sGRT value to agnostic value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sGRT has 9 decimals.
     */
    ...
    /**
        @notice Convert agnostic value at current index to flat sGRT value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sGRT has 9 decimals.
     */
    ...
    /**
        @notice Convert flat sGRT value to agnostic value at a given index value
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e9 is because sGRT has 9 decimals.
     */
